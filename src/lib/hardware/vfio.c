...
{
    printf ("mmap_memory (%d): %p %" PRIu64 " %" PRIu64 " %d %d\n",
            _container, buffer, size, iova, read, write);
    if (!_container) return 0;

    if (buffer == NULL) {
        printf ("no buffer... ");
        buffer = mmap(0, size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, 0, 0);
        printf ("buffer: %p\n", buffer);
    }
...
//         assert(ioctl(device, VFIO_DEVICE_GET_REGION_INFO, &reg)==0);
        {
           ...
           if (ret != 0) {
              printf ("can't get region %d info\n", i);
              continue;
           }
        }
...
//         assert(ioctl(device, VFIO_DEVICE_GET_IRQ_INFO, &irq)==0);
        {
           int ret = ioctl(device, VFIO_DEVICE_GET_IRQ_INFO, &irq);
           if (ret != 0) {
              printf ("can't get interrupt %d info\n", i);
              continue;
           }
        }
...
int mask_irq(int device, int n, int subn)
{
    struct vfio_device_info device_info = { .argsz = sizeof(device_info) };
//     struct vfio_irq_info irqinfo = { .argsz = sizeof(irqinfo) };
    struct vfio_irq_set irq = { .argsz = sizeof(irq) };

    assert(ioctl(device, VFIO_DEVICE_GET_INFO, &device_info)==0);
    if (n >= device_info.num_irqs) return -1;

//     reg.index = n;
//     assert(ioctl(device, VFIO_DEVICE_GET_IRQ_INFO, &irqinfo)==0);

    irq.index = n;
    irq.start = subn;
    irq.count = 0;
    irq.flags = VFIO_IRQ_SET_DATA_NONE | VFIO_IRQ_SET_ACTION_TRIGGER;
    assert_perror(ioctl(device, VFIO_DEVICE_SET_IRQS, &irq));
    return 0;
}
